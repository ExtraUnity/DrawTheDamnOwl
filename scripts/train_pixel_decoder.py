...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Path to the embedding archive used by the latent transition model. "
            "If omitted, the script uses the archive recorded in --latent-metrics-json."
        ),
    )
    parser.add_argument(
        "--transitions-csv",
    ...
    parser.add_argument("--min-src-stage", type=int, default=3, help="Minimum source stage to train")
    parser.add_argument("--max-src-stage", type=int, default=6, help="Maximum source stage to train")
    parser.add_argument(
        "--output-mode",
    ...
    parser.add_argument("--edge-max-stage", type=int, default=3, help="Apply edge loss through this source stage")
    parser.add_argument(
        "--change-weight",
        type=float,
        default=8.0,
...
def load_latent_metadata(path: Path) -> Dict[str, Any]:
    if not path.exists():
        return {}
...
def resolve_embeddings_npz_path(cli_value: str, latent_metadata: Dict[str, Any]) -> Path:
    cli_value = str(cli_value).strip()
    if cli_value:
        return Path(cli_value)

    metrics_value = str(latent_metadata.get("embeddings_npz", "")).strip()
    if metrics_value:
        return Path(metrics_value)

    return LEARNING_ROOT / "embeddings" / "clip_embeddings_all.npz"
...
def build_latent_model(
    ...
    latent_metadata = load_latent_metadata(config_path)
    expected_embedding_dim = latent_metadata.get("embedding_dim")
    trained_embeddings_npz = str(latent_metadata.get("embeddings_npz", "")).strip()
    if expected_embedding_dim is not None and int(expected_embedding_dim) != int(embedding_dim):
        trained_label = trained_embeddings_npz or "unknown embedding archive"
        raise RuntimeError(
            "Latent transition checkpoint and embedding archive do not match. "
            f"The latent model in {checkpoint_path} was trained with embedding_dim={int(expected_embedding_dim)} "
            f"from {trained_label}, but --embeddings-npz={embeddings_npz_path} has embedding_dim={embedding_dim}. "
            "Use the same embedding family for both. "
            "For the checked-in data/owl_output/learning/ar_baseline artifacts, that means DINO embeddings rather than CLIP."
        )
    ...
    load_transition_mlp_checkpoint(model, checkpoint_path, device)
    model.eval()
...
def main() -> None:
    ...
    latent_metadata = load_latent_metadata(Path(args.latent_metrics_json))
    embeddings_npz_path = resolve_embeddings_npz_path(args.embeddings_npz, latent_metadata)
    lookup, embedding_dim = load_embedding_lookup(embeddings_npz_path)
    ...
    latent_model = build_latent_model(
        Path(args.latent_metrics_json),
        Path(args.latent_checkpoint),
        embedding_dim,
        embeddings_npz_path,
        device,
    )
    ...
    summary = {
        "embeddings_npz": str(embeddings_npz_path),
